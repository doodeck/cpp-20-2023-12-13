...
public:
    ...
    TaskResumer(CoroutineHandle coro_hndl) : coro_hndl_{coro_hndl}
    {
    }
...
TEST_CASE("first coroutine")
{
    ...
    while(task.resume())
    {
        std::cout << "From caller: foo() suspended...\n";
    }
...
TEST_CASE("coroutine + generator")
{
    auto gen_square = squares_gen(10);

    for(auto it = gen_square.begin(); it != gen_square.end(); ++it)
    {
        std::cout << *it << "\n";
    }
}
